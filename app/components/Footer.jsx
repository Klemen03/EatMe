...
function Footer() {
  return (
    <footer className="border-t-2 mt-20 border-gray-600 bg-yellow-100 dark:bg-black absolute bottom-0 left-0 right-0">
      <div className="flex flex-col items-center max-w-6xl gap-6 lg:pb-11 mx-auto">
        <div className="flex flex-row justify-center gap-12 pt-5  w-full">
          {icons.map((icon) => (
            <div
        ...
        <div className="flex flex-row gap-8 cursor-pointer box-content">
          {links.map((link) => (
            <div className="hover:text-green-500 underline" key={link.id}>
              {link.name}
            </div>
          ))}
        </div>
